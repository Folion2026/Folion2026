...
async function route(req:IncomingMessage,res:ServerResponse){
 ...
 if(req.method==='PATCH'&&narrativeReviewMatch){
  ...
  const {error}=await admin.from('narrative_sections').update(values).eq('workspace_id',workspace.id).eq('id',section.id);if(error)throw error;await admin.from('projects').update({knowledge_status:'Review needed',updated_by:user.id}).eq('workspace_id',workspace.id).eq('id',section.project_id);await audit(workspace.id,user.id,auditAction,'narrative_section',section.id,{projectId:section.project_id});return reply(res,200,await ingestionPayload(workspace.id,section.project_id))
 }
