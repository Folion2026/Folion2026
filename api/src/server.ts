...
async function markDependentNarrativesStale(workspaceId:string,projectId:string,itemId:string){
 const {data,error}=await admin.from('narrative_sections').select('id,supporting_item_ids').eq('workspace_id',workspaceId).eq('project_id',projectId).in('status',['draft','approved']);if(error)throw error
 const ids=(data||[]).filter(section=>(section.supporting_item_ids||[]).includes(itemId)).map(section=>section.id);if(!ids.length)return
 const {error:updateError}=await admin.from('narrative_sections').update({needs_refresh:true,stale_reason:'Supporting project knowledge changed.'}).in('id',ids);if(updateError)throw updateError
}
...
async function route(req:IncomingMessage,res:ServerResponse){
 ...
 if(req.method==='PATCH'&&itemReviewMatch){
  ...
  if(action==='edit_accept'||action==='reject')await markDependentNarrativesStale(workspace.id,item.project_id,item.id)
 ...
 if(req.method==='PATCH'&&narrativeReviewMatch){
  ...
  if((action==='approve'||action==='edit_approve')&&section.needs_refresh)return fail(res,409,'Needs refresh — supporting project knowledge changed.')
  if(action==='approve'||action==='edit_approve'){const supporting=Array.isArray(section.supporting_item_ids)?section.supporting_item_ids:[];if(supporting.length){const {data:invalid,error:invalidError}=await admin.from('knowledge_items').select('id').in('id',supporting).in('status',['rejected','superseded']);if(invalidError)throw invalidError;if(invalid?.length)return fail(res,409,'Needs refresh — supporting project knowledge changed.')}}
