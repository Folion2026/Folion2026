...
export function approvedDraftSections(project:Project){const existing=(project.knowledge?.draft||[]).filter(section=>section.approved&&Boolean(section.value));const mapping:Record<NarrativeSectionType,{key:DraftKey;label:string}>={project_summary:{key:'summary',label:'Project summary'},challenge_opportunity:{key:'challenge',label:'Challenge or opportunity'},response:{key:'response',label:'Response'},outcome_future_relevance:{key:'outcome',label:'Outcome / future relevance'},distinctive_response:{key:'response',label:'Distinctive response'},precedent_strength:{key:'outcome',label:'Precedent strength'},future_relevance:{key:'outcome',label:'Future relevance'},precedent_relevance:{key:'outcome',label:'Precedent strength and future relevance'},project_narrative:{key:'outcome',label:'Project narrative'}};const basis:Record<NarrativeBasisType,DraftBasis>={source_supported:'facts',team_input:'team',mixed:'both'};const ingested=(project.approvedNarratives||[]).map(item=>({key:mapping[item.sectionType].key,label:mapping[item.sectionType].label,value:item.text,approved:true,basis:basis[item.basisType]}));const seen=new Set<string>();return[...existing,...ingested].filter(section=>{const key=`${section.key}|${section.value}`;if(seen.has(key))return false;seen.add(key);return true})}
