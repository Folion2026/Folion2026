import {ApprovedEvidenceItem,ApprovedNarrativeSection,Asset,AssetType,DraftBasis,DraftKey,Evidence,EvidenceCategory,ExternalLinkProvider,FolionDraftSection,KnowledgeFact,KnowledgeFactKey,KnowledgeItem,KnowledgeReviewStatus,KnowledgeSourceType,NarrativeBasisType,NarrativeSectionType,PlaceFramework,Project,ProjectConfidentiality,ProjectExternalLink,ProjectIdentity,ProjectKnowledge,ProjectMetrics,ProjectOutcome,SearchIntelligence,SourceLifecycle,Story,StudioAssets,TeamInput} from '../types'
...
const sourceLifecycles:SourceLifecycle[]=['uploaded','analysing','extracted','pending_review','approved','deleted']
const externalLinkProviders:ExternalLinkProvider[]=['SharePoint','OneDrive','Google Drive','Dropbox','Box','Other']
...
function normalizeAsset(value:unknown,index:number):Asset|null{
 ...
 return{id:text(asset.id,`asset-${index}`),type,title:text(asset.title,`Project asset ${index+1}`),caption:text(asset.caption),url:text(asset.url),storagePath:text(asset.storagePath)||undefined,fileName:text(asset.fileName)||undefined,mimeType:text(asset.mimeType)||undefined,fileSize:typeof asset.fileSize==='number'?asset.fileSize:undefined,uploadedAt:text(asset.uploadedAt)||undefined,uploadedBy:text(asset.uploadedBy)||undefined,sourcePage:typeof asset.sourcePage==='number'?asset.sourcePage:null,tags:strings(asset.tags),uploadedCategory:text(asset.uploadedCategory,type),isPrimary:Boolean(asset.isPrimary),isSelectedForGallery:asset.isSelectedForGallery!==false,retentionKind:asset.retentionKind==='temporary_source'?'temporary_source':'durable',lifecycleStatus:sourceLifecycles.includes(asset.lifecycleStatus as SourceLifecycle)?asset.lifecycleStatus:undefined,sourceDeletedAt:text(asset.sourceDeletedAt)||undefined}
}

function normalizeExternalLinks(value:unknown):ProjectExternalLink[]{return records(value).map(item=>({id:text(item.id),projectId:text(item.projectId),label:text(item.label,'Project folder'),url:text(item.url),provider:externalLinkProviders.includes(item.provider as ExternalLinkProvider)?item.provider as ExternalLinkProvider:'Other',isPrimary:Boolean(item.isPrimary)})).filter(item=>item.id&&item.projectId&&item.url)}
...
const narrativeTypes:NarrativeSectionType[]=['project_summary','challenge_opportunity','response','outcome_future_relevance','distinctive_response','precedent_strength','future_relevance']
...
export function reviewedKnowledgeFacts(project:Project){const existing=(project.knowledge?.facts||[]).filter(fact=>fact.status==='reviewed'&&Boolean(fact.value));const labels=new Map(KNOWLEDGE_FACTS.map(item=>[item.key,item.label]));const ingested=(project.approvedEvidence||[]).flatMap(item=>{const normalised=(item.structuredField||item.field).toLowerCase().trim().replace(/[^a-z0-9]+/g,'_').replace(/^_|_$/g,'');const key=evidenceFieldKeys[normalised]||({sector_typology:'sector',dwellings_units:'dwellings',height_levels:'height',explicit_responsibilities:'scope'} as Record<string,KnowledgeFactKey>)[normalised];return key?[{key,label:labels.get(key)||item.field,value:item.value,sourceType:'uploaded-asset' as const,status:'reviewed' as const}]:[]});const seen=new Set<string>();return[...existing,...ingested].filter(fact=>{const key=`${fact.key}|${fact.value}`;if(seen.has(key))return false;seen.add(key);return true})}
export function approvedDraftSections(project:Project){const mapping:{[key:string]:{key:DraftKey;label:string}}={project_summary:{key:'summary',label:'Project summary'},project_narrative:{key:'outcome',label:'Project narrative'}};const basis:Record<NarrativeBasisType,DraftBasis>={source_supported:'facts',team_input:'team',mixed:'both'};const ingested=(project.approvedNarratives||[]).flatMap(item=>{const target=mapping[item.sectionType];return target?[{...target,value:item.text,approved:true,basis:basis[item.basisType]}]:[]});const seen=new Set<string>();return ingested.filter(section=>{const key=`${section.key}|${section.value}`;if(seen.has(key))return false;seen.add(key);return true})}
