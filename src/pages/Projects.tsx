...
export default function Projects(){
  ...
  const publishableCount=activeProjects.filter(project=>project.confidentiality==='publicly-publishable').length
  ...
  const results=useMemo(()=>activeProjects.filter(project=>{
    ...
    const matchesFilter=filter==='All'||(filter==='Internal only'&&project.confidentiality==='internal-only')||(filter==='Externally shareable'&&project.confidentiality==='externally-shareable')||(filter==='Publicly publishable'&&project.confidentiality==='publicly-publishable')||project.sector===filter
  ...
  return <div>
    ...
    <section className="projects-search-panel">
      ...
      <div className="search-panel-footer"><FilterChips active={filter} onChange={setFilter}/><div className="visibility-tally"><span><Lock/> {internalCount} internal</span><span><Share2/> {externalCount} external</span><span><Globe2/> {publishableCount} publishable</span></div></div>
