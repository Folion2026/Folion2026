...
import {FALLBACK_PROJECT_IMAGE} from '../lib/project'
...
export default function ProjectReflectionPage(){
 ...
 const chooseFiles=(type:AssetType)=>(event:ChangeEvent<HTMLInputElement>)=>{const selected=Array.from(event.target.files||[]).map(file=>({name:file.name,url:file.type.startsWith('image/')?URL.createObjectURL(file):''}));setUsingSample(false);setUploads(current=>({...current,[type]:[...(current[type]||[]),...selected]}));event.target.value=''}
 const addSamples=()=>{setUsingSample(true);setUploads({report:[{name:'Project report.pdf',url:''}],hero:[{name:'Warrin civic rooms hero.jpg',url:FALLBACK_PROJECT_IMAGE}]})}
 ...
 const createProject=()=>{const projectAssets=assets();const hasSampleReport=usingSample&&Boolean(projectAssets.some(asset=>asset.type==='report'));const source=hasSampleReport?extracted:{projectName:'',company:'',location:'',sector:'',year:'',client:'',status:'',brief:''};const coverImage=projectAssets.find(asset=>asset.type==='hero')?.url||FALLBACK_PROJECT_IMAGE;const id=`imported-project-${Date.now().toString().slice(-6)}`;const project:Project={id,...source,visibility:'private',siteArea:'',gfa:'',height:'',services:[],team:[],story:{brief:source.brief,challenge:teamInput.challengeOpportunity,response:teamInput.teamResponse,outcome:'',lessons:teamInput.futureRelevance},knowledge:{facts:[],teamInput,draft:[]},assets:projectAssets,tags:[],coverImage};addProject(project);localStorage.removeItem(teamInputStorageKey);navigate(`/projects/${id}?tab=knowledge`)}
