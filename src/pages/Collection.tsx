...
export default function CollectionPage(){
 ...
 const items=collection.projectIds.map(projectId=>safeProjects.find(project=>project.id===projectId)).filter((project):project is NonNullable<typeof project>=>Boolean(project)),save=async(value:CollectionDraft)=>{await updateCollection({...value,id:collection.id})}
 const remove=async()=>{if(!window.confirm('Delete this collection? Projects will not be deleted.'))return;setDeleting(true);setDeleteError('');try{await deleteCollection(collection.id);navigate('/collections',{replace:true,state:{notice:'Collection deleted. Projects were preserved.'}})}catch(reason){setDeleteError(reason instanceof Error?reason.message:'Unable to delete this collection');setDeleting(false)}}
 return <div>
  <button onClick={()=>navigate('/collections')} className="detail-back mb-8"><ArrowLeft size={16}/> Back to Collections</button>
  <header className="collection-header"><div><p className="eyebrow">Curated portfolio</p><h1>{collection.name}</h1><p>{collection.brief}</p></div>{workspaceRole!=='viewer'&&<div className="flex flex-wrap gap-2"><Button onClick={()=>setEditing(true)} disabled={deleting}><Edit3 size={16}/> Edit Collection</Button><Button variant="ghost" onClick={()=>void remove()} disabled={deleting}><Trash2 size={16}/> {deleting?'Deleting…':'Delete Collection'}</Button></div>}</header>
