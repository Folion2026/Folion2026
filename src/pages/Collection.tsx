...
export default function CollectionPage(){
 ...
 const {collections,projects,renameCollection,toggleProjectInCollection}=useStore()
 ...
 const saveName=()=>{if(name.trim())renameCollection(collection.id,name.trim());setRenaming(false)}

 return <div>
  <button onClick={()=>navigate('/projects')} className="detail-back mb-8"><ArrowLeft size={16}/> Back to projects</button>
  <header className="collection-header">
   <div><p className="eyebrow">Curated portfolio</p>{renaming?<div className="collection-title-edit"><input autoFocus value={name} onChange={event=>setName(event.target.value)} onKeyDown={event=>event.key==='Enter'&&saveName()}/><button onClick={saveName}><Check size={18}/></button><button onClick={()=>{setName(collection.name||'');setRenaming(false)}}><X size={18}/></button></div>:<h1>{collection.name||'Untitled collection'}</h1>}<p>{collection.description||'A curated group of project memory.'}</p></div>
