...
export function StoreProvider({children}:{children:ReactNode}){
 ...
 const addPerson=useCallback(async(person:Omit<Person,'id'>)=>{if(!session||!workspace)throw new Error('A workspace session is required');try{const result=await apiRequest<{person:Person}>(session,'/v1/people',{method:'POST',body:JSON.stringify({workspaceId:workspace.id,person})});setPeople(current=>[...current.filter(item=>item.id!==result.person.id),result.person].sort((a,b)=>a.name.localeCompare(b.name)));setError('');return result.person}catch(reason){setError(reason instanceof Error?reason.message:'Unable to add the person');throw reason}},[session,workspace])
