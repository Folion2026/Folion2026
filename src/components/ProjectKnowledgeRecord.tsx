...
export default function ProjectKnowledgeRecord({project,onUpdate}:{project:Project;onUpdate:(project:Project)=>void}){
 const navigate=useNavigate();const safe=normalizeProject(project);const [editingKey,setEditingKey]=useState<KnowledgeFactKey|null>(null);const [editValue,setEditValue]=useState('')
 ...
 return <div className="knowledge-record">
  ...
  <ConfidentialityControl value={safe.confidentiality} onChange={confidentiality=>onUpdate({...safe,confidentiality,visibility:confidentiality==='publicly-publishable'?safe.visibility:'private'})} compact/>
