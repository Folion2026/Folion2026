...
export default function CollectionForm({initial,projects,onSave,onCancel}:{initial?:CollectionDraft;projects:Project[];onSave:(value:CollectionDraft)=>Promise<void>;onCancel:()=>void}){
 ...
 return <div className="collection-form space-y-5">
  ...
  <fieldset><legend className="label">Selected Projects</legend><div className="collection-form-projects">{safeProjects.map(project=><button type="button" key={project.id} className={value.projectIds.includes(project.id)?'selected':''} onClick={()=>toggle(project.id)}><img src={project.coverImage||FALLBACK_PROJECT_IMAGE} alt=""/><span><strong>{project.projectName}</strong><small>{[project.sector,project.year].filter(Boolean).join(' · ')}</small></span>{value.projectIds.includes(project.id)&&<Check/>}</button>)}</div></fieldset>
