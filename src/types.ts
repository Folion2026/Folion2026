...
export type SourceLifecycle='uploaded'|'analysing'|'extracted'|'pending_review'|'approved'|'deleted'
export interface Asset{id:string;type:AssetType;title:string;caption:string;url:string;storagePath?:string;fileName?:string;mimeType?:string;fileSize?:number;uploadedAt?:string;uploadedBy?:string;sourcePage:number|null;tags:string[];uploadedCategory:string;isPrimary:boolean;isSelectedForGallery:boolean;retentionKind?:'durable'|'temporary_source';lifecycleStatus?:SourceLifecycle;sourceDeletedAt?:string}
export type ExternalLinkProvider='SharePoint'|'OneDrive'|'Google Drive'|'Dropbox'|'Box'|'Other'
export interface ProjectExternalLink{id:string;projectId:string;label:string;url:string;provider:ExternalLinkProvider;isPrimary:boolean}
